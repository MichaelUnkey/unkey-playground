...
// Step 1 Create your first key
  ...
  const step1Messages: Message[] = [
    ...
    { content: "Welcome to the Unkey playground. Here, you can test how Unkey's API works using curl commands. The first step is to create a key for a user. They will use this key to authenticate with your API. Normally, you would need an apiId and root key, but we've already set this up for you. Usually, this setup would be done in your Unkey dashboard. For now, leave the <token> in place. This is where your bearer token or root key would go. For each step, type in or copy the curl command below into the input at the bottom and press enter. Alternatively, you can just click the curl command and it will be sent off for you.", 
    color: "text-white" 
    },
    { content: `As you can see its a typical POST request with a url, headers and data. The Bearer in the first header authenticates you in the Unkey system. The data object passes what api you would like to create a key for.`, 
    color: "text-white" 
    },
    {
...
// Step 2 Get the key we just created
  ...
  const step2Messages: Message[] = [
    { content: `Nice Job! Now that we have created a user key let us use the getKey endpoint to get some info. A typical key will have useful information like roles, permissions, remaining uses, ownerId and more. A full list can be found on our docs https://www.unkey.com/docs/api-reference/keys/get`, color: "text-white" },
  ];
...
// Step 3 Verify the key
  ...
  const step3Messages: Message[] = [
    { content: `lets go! Looks like that worked and returned some data. This object can get much larger depending on the options added to the key. Now we can verify the key we just created to make sure it will work for a user. Each verification will add some analytics data we will get in a later step.`, color: "text-white" },
  ];
...
//Step 5 Verify the key again
  ...
  const step5Messages: Message[] = [
    { content: `If this worked correctly you should get back a {} in response. This is normal and signifies success. If there was an error you would have gotten back an error in stead. Lets now verify the key just to make sure your key is updated with the ownerId. Just to give you piece of mind.`, color: "text-white" }
  ];
...
// Step 7 Verify the key again
  ...
  const step7Messages: Message[] = [
    { content: `Again we should have gotten {} in response. Lets verify the key to make sure expiration was set correctly. It will also give up more analytics data for the next step.`, color: "text-white" },
  ];
...
// Step 8
  ...
  const step8Messages: Message[] = [
    { content: `Nice! it looks like the expires value now holds the value you entered. Now lets see the analytics data on our key. This will show how many times the key was successfuly verified, ratelimited, or the usage has been exceeeded.`, color: "text-white" }
    ];
...
//Step 9
  ...
  const step9Messages: Message[] = [
    { content: `If it was a new key and all our verify keys worked correctly. success should show a value of 3. Lets say we no longer want this key to have any access. While we can use updateKey to set enabled to false. Here lets delete the key so it has not option of being used again. Maybe the key was leaked to a bad actor or it is no longer needed for any reason.`, color: "text-white" },
  ];
...
//Step 10 Last Verification
  ...
  const step10Messages: Message[] = [
    { content: `Now lets verify that deleting a key worked as it should. Use the verifyKey route to check the key again. This should now return valid: false meaning this key is no longer a valid Unkey key.`, color: "text-white" },
];
...
//Step 11 Congrats!
  ...
  const step11Messages: Message[] = [
    { content: `Nice job. I hope this was useful in some way on learning about some Unkey basics. To learn more visit our docs. To get started feel free to setup an unkey dashboard for your project.`, color: "text-white" },
  ];
