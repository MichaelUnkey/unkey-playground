  ...
  return (
    ...
    >
      <h2 className="text-lg mt-6">Welcome to the Unkey API playground</h2>
        <p className="text-md mt-2">Here you can test out the Unkey API using curl commands to make Unkey API calls.</p>
      <h2 className="text-2xl">Create a first Key</h2>
      <p>
        Directions: Type commands in manualy, copy and paste, or just click the
        &quot;Send request&quot; button under each step. Leave the <span className=" text-white">&lt;token&gt;</span> tag
        for now. Normally you would put your root key here. We will handle any
        rootkey requirements in this playground.
      </p>
      <p>
...
export function Step3(
  ...
  return (
    <div
      key="step3"
      className={cn(step === 3 ? "flex clex-col" : "hidden", "gap-4")}
    >
      <h2>Verify Key</h2>
      <div>
        <div className="flex flex-col gap-4 ">
    ...
    <div key="step10" className={cn(step === 10 ? "flex" : "hidden")}>
      ...
      <div>
        ...
        <div className="flex flex-row justify-end">
          <Button onClick={() => handleButtonClick(10)} variant={"outline"}>
            Send request
          </Button>
        </div>
      </div>
...
export function Step11(step: number, stepData: stepDataType) {
  return (
    <div key="step11" className={cn(step === 11 ? "flex flex-col" : "hidden", "w-full")}>
